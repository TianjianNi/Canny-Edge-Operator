...
def gaussian_mask_seven_by_seven():
    ...
    row4 = np.array([2, 2, 4, 8, 4, 2, 2])
    row5 = np.array([1, 2, 2, 4, 2, 2, 1])
    row6 = np.array([1, 1, 2, 2, 2, 1, 1])
...
class CannyEdgeDetector:
    ...
    def non_maxima_suppression(self):
        ...
        size_i, size_j = magnitudes.shape
        ...
        for row in range(1, size_i - 1):
            for col in range(1, size_j - 1):
                ...
                if quantized_angles[row][col] == 0:
                    if magnitudes[row][col + 1] < local_magnitude and magnitudes[row][col - 1] < local_magnitude:
                        output_non_maxima_suppression[row][col] = local_magnitude
                elif quantized_angles[row][col] == 1:
                    if magnitudes[row - 1][col + 1] < local_magnitude and magnitudes[row + 1][col - 1] < local_magnitude:
                        output_non_maxima_suppression[row][col] = local_magnitude
                elif quantized_angles[row][col] == 2:
                    if magnitudes[row - 1][col] < local_magnitude and magnitudes[row + 1][col] < local_magnitude:
                        output_non_maxima_suppression[row][col] = local_magnitude
                elif quantized_angles[row][col] == 3:
                    if magnitudes[row - 1][col - 1] < local_magnitude and magnitudes[row + 1][col + 1] < local_magnitude:
                        output_non_maxima_suppression[row][col] = local_magnitude
